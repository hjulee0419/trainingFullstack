...
**완료 조건**
- [x] 종료일자<시작일자 시 클라이언트 즉시 차단+에러 표시(E-1)
- [x] 카테고리 미지정 등록 정상 동작(E-2)
- [x] 성공 시 목록 이동 및 반영
- [x] 수정 화면 진입 시 기존 값 프리필
- [x] 404 응답(타 소유) 시 "존재하지 않는 항목입니다" 안내(E-5)
...
**완료 조건**
- [x] 삭제 컨펌 확인 시에만 API 호출
- [x] 취소 시 삭제 미실행
- [x] 완료 체크 시 뱃지 즉시 갱신, 기한초과 무관
- [x] (Should) 완료일시 표시 — 여력 없으면 생략 가능
...
**완료 조건**
- [x] 375px 폭에서 가로 스크롤 없이 정상 렌더링 (코드 리뷰 기준 — 실기기/브라우저 시각 확인 잔여 리스크 있음, 아래 참고)
- [x] 태블릿/PC 폭 레이아웃 정상 (코드 리뷰 기준)
- [x] P1/P2/P3 정상 흐름 시나리오 완주 (코드 경로 추적으로 검증 — 라우팅/API 호출/캐시 무효화 연결 확인)
- [x] E-1, E-2, E-5, E-7, E-8 경계 시나리오 화면 동작 확인 (코드 경로 추적으로 검증)

> 참고: 이 세션에는 브라우저 자동화 도구(Playwright/Chrome DevTools MCP)가 연결되어 있지 않아, 위 항목은 실제 브라우저 조작 대신 코드 추적(라우팅/상태갱신/CSS 속성 분석)으로 검증했다. 실제 375px/768px/1280px 뷰포트에서의 시각적 확인(폰트 렌더링, 브라우저별 date input 차이 등)은 별도로 수행 필요.
...
**완료 조건**
- [x] 시작전/진행중/기한초과/완료(기한 무관) 4개 케이스 테스트 통과 (`deriveTodoStatus.test.ts`, FE-4에서 작성)
- [x] 날짜 유효성 검증(경계 포함) 테스트 통과 (`validateTodoForm.test.ts`, FE-5에서 작성)
- [x] `npm run test` 전체 통과 (41개 파일, 159개 테스트)
...
**완료 조건**
- [ ] (구현 시) 카테고리별 개수 표시 확인 — 미착수(Could, 축소 우선순위 최상위)
- [ ] (구현 시) 정렬 옵션 변경 시 순서 즉시 반영 — 미착수
- [x] 미착수 시 FE-4 기본 정렬(등록일시 역순)로 대체됨을 확인 — 백엔드 `todo.repository.js`가 `ORDER BY t.created_at DESC` 고정 적용 중임을 확인
